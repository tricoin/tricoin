...
{
    // Tricoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Some adjustments to the start of the lifetime to Tricoin
    if (nHeight < 42000) {
        nSubsidy = 250 * COIN;
        ...
        nSubsidy >>= (nHeight / 306600); // Tricoin: 306600 blocks in ~2 years
    }
...
static const int64 nTargetTimespan = 86400; //420 * 205.2; = 86184 // Tricoin: 420 blocks
static const int64 nTargetSpacing = 120;//3.42 * 60; // Tricoin: 2 minutes
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    /* current difficulty formula, Tricoin - kimoto gravity well */
...
unsigned int static OldGetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tricoin difficulty adjustment protocol switch (Thanks to FeatherCoin for this idea)
    ...
    // Tricoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tricoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TricoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TricoinMiner : ProcessBlock, block not accepted");
    }
